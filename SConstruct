...
opts.Add('destdir', 'Staging area to install FirstDrive to.  Useful for packagers. ', '')
...
#------------#
    ...
    default_settingsdir = 'Library/Preferences/FirstDrive'
    default_prefix = "/Applications/FirstDrive"
...
#-------------#
    ...
    env = Environment(ENV = os.environ,
        CPPPATH = ['#src'],
        CCFLAGS = ['-std=c++11', '-Wall', '-Wextra'],
        LIBPATH = ['.', '#lib', '/usr/lib','/usr/local/lib', '/opt/lib'],
        LINKFLAGS = ['-pthread', '-lFestival', '-lestools', '-lestbase', '-leststring'], #, '-lncurses', '-lreadline'],
        CC = 'gcc', CXX = 'g++',
        options = opts)
    ...
    #, 'speech_tools/EST.h', 'speech_tools/EST_String.h', 'festival/festival.h'] 
    check_libs = []
...
opts.Add('datadir', 'Path suffix where where FirstDrive data will be installed', default_datadir) 
opts.Add('localedir', 'Path where FirstDrive locale will be installed', default_localedir)
opts.Add('bindir', 'Path suffix where FirstDrive binary executable will be installed', default_bindir)
...
Type: 'scons' to compile with the default options.
      ...
      'scons settings=.FirstDrive' to change settings directory.
      'scons install' (as root) to install FirstDrive.
...
if 'install' in COMMAND_LINE_TARGETS:
    if not os.path.isfile('data/SConscript'):
        raise 'FirstDrive data not found. Please make sure data is placed in vdrift directory. See README.md and http://wiki.vdrift.net.' 

...
//	Specific for FirstDrive
		case 'w':
			...
			l = track.Collide(cam.position.ScaleR(-1.0), tvec2, tvec1, true, tempstrip, colbez);
			if (!l)	 {
				mq1.AddMessage("Can't create STOP marker: no road found under cursor");
			}
			else {
				track.AddStopSequence(colbez);
				mq1.AddMessage("Added STOP marker.");
			}
			break;
		...
		case 'q':
			...
			l = track.Collide(cam.position.ScaleR(-1.0), tvec2, tvec1, true, tempstrip, colbez);
			if (!l)	{
				mq1.AddMessage("Can't create GIVEWAY marker: no road found under cursor");
			}
			else	{
				track.AddCedaSequence(colbez);
				mq1.AddMessage("Added GIVEWAY marker.");
			}
			break;
		...
		case 'p':
			...
			l = track.Collide(cam.position.ScaleR(-1.0), tvec2, tvec1, true, tempstrip, colbez);
			if (!l)	{
				mq1.AddMessage("Can't create PARK marker: no road found under cursor");
			}
			else	{
				track.AddParkSequence(colbez);
				mq1.AddMessage("Added PARK marker.");
			}
			break;
...
*/
	...
	if (argc > 2) { //set custom resolutions
		w = atoi(argv[1]);
		h = atoi(argv[2]);
	} else {
		printf("\n\n\nYou can set custom resolutions as ./build/vdrift WIDTH HEIGHT. For instance:\n");
		printf("\t$ ./build/vdrift 1920 1080\n\nRunning at %dx%d...\n\n\n", w, h);
	}

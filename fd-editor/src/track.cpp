...
void TRACK::Write(string trackname)
{
	...
	count = 0;
	lapsize = stop.size();
	trackconfig.SetParam("number of stops", lapsize);
	for (vector <LAPAREA>::iterator i = stop.begin(); i != stop.end(); i++,count++)
	{
		stringstream lapname;
		lapname << "stop " << count;
		VERTEX laps;
	...
	count = 0;
	lapsize = ceda.size();
	trackconfig.SetParam("number of giveways", lapsize);
	for (vector <LAPAREA>::iterator i = ceda.begin(); i != ceda.end(); i++,count++)
	{
		stringstream lapname;
		lapname << "giveway " << count;
		VERTEX laps;
		laps.Set(i->roadidx, i->patchidx, 0);
		trackconfig.SetParam(lapname.str(), laps.v3());
	}
	...
	count = 0;
	lapsize = park.size();
	trackconfig.SetParam("number of parking spots", lapsize);
	for (vector <LAPAREA>::iterator i = park.begin(); i != park.end(); i++,count++)
	{
		stringstream lapname;
		lapname << "park spot " << count;
		VERTEX laps;
		laps.Set(i->roadidx, i->patchidx, 0);
		trackconfig.SetParam(lapname.str(), laps.v3());
	}
...
void TRACK::Load(string trackname)
{
	...
	lapmarkers=0;
	trackconfig.GetParam("number of stops", lapmarkers);
	for (int l = 0; l < lapmarkers; l++)
	{
		float lapraw[3]= {0.,0.,0.};
		stringstream lapname;
		...
		newarea.roadidx  = (int) lapraw[0];
		newarea.patchidx = (int) lapraw[1];
		stop.push_back(newarea);
	}

	lapmarkers=0;
	trackconfig.GetParam("number of giveways", lapmarkers);
	for (int l = 0; l < lapmarkers; l++)
	{
		float lapraw[3]= {0.,0.,0.};
		stringstream lapname;
		...
		newarea.roadidx  = (int) lapraw[0];
		newarea.patchidx = (int) lapraw[1];
		ceda.push_back(newarea);
	}

	lapmarkers=0;
	trackconfig.GetParam("number of parking spots", lapmarkers);
	for (int l = 0; l < lapmarkers; l++)
	{
		float lapraw[3]= {0.,0.,0.};
		stringstream lapname;
...
void TRACK::AddStopSequence(BEZIER * patch){
	bool found = false;
	ROADSTRIPNODE * curnode = roads;
	...
	if (found)	{
		...
		cerr << "Error finding bezier for STOP sequence" << endl;
	}
}
void TRACK::AddCedaSequence(BEZIER * patch){
	bool found = false;
	ROADSTRIPNODE * curnode = roads;
	...
	if (found)	{
		...
		cerr << "Error finding bezier for GIVEWAY sequence" << endl;
	}
}

void TRACK::AddParkSequence(BEZIER * patch){
	bool found = false;
	ROADSTRIPNODE * curnode = roads;
	...
	if (found)	{
		...
		cerr << "Error finding bezier for PARKING sequence" << endl;
	}
}

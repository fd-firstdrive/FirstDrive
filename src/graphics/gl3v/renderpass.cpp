...
{
	/*render(gl, 0, 0, w, h, stringMap, drawList, sharedTextures, errorOutput);
}	

bool RenderPass::render(GLWrapper & gl, unsigned int ow, unsigned int oh, unsigned int w, unsigned int h, StringIdMap & stringMap, const std::vector <const std::vector <RenderModelExt*>*> & externalModels, const NameTexMap & sharedTextures, std::ostream & errorOutput)
{*/
	if (!enabled)
		return false;
	...
	// Set viewport.
	//gl.Viewport(ow, oh, width, height);

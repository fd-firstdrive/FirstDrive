...
/*
	render(0, 0, w, h, stringMap, externalModels, errorOutput);
}

void Renderer::render(unsigned int ow, unsigned int oh, unsigned int w, unsigned int h, StringIdMap & stringMap, const std::map <StringId, std::vector <RenderModelExt*> > & externalModels, std::ostream & errorOutput)
{*/
	for (std::vector <RenderPass>::iterator i = passes.begin(); i != passes.end(); i++)
	{
		// Extract the appropriate map and generate a drawlist.passes
...
/*
void Renderer::renderRVM(unsigned int w, unsigned int h, StringIdMap & stringMap, const std::map <StringId, std::map <StringId, std::vector <RenderModelExt*> *> > & externalModels, std::ostream & errorOutput){
	for (std::vector <RenderPass>::iterator i = passes.begin(); i != passes.end(); i++){

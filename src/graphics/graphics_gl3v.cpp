...
void GraphicsGL3::DrawScene(std::ostream & error_output, int offset_w, int offset_h, int w2, int h2){
	// reset active vertex array in case it has been modified outside
	gl.unbindVertexArray();

	gl.logging(logNextGlFrame);
	if (offset_w != 0 and offset_h != 0 and offset_w != offset_h) {
		renderer.render(offset_w, offset_h, stringMap, drawMap, error_output);
	} 
		...
		wo = w ; ho = h;
		w = w2 ; h = h2;
		//ReloadShaders(info_output, error_output); //glViewport (offset_w+20, offset_h+20, w2-20, h2-20);
		renderer.render(w2, h2, stringMap, drawMap, error_output);

		w = wo ; h = ho;
		//ReloadShaders(info_output, error_output); //glViewport (0, 0, wo, ho);
	}
	gl.logging(false);
...
/*
void GraphicsGL3::DrawSceneRVM(std::ostream & error_output){
	// reset active vertex array in case it has been modified outside
	gl.unbindVertexArray();

	gl.logging(logNextGlFrame);
	renderer.renderRVM(wr, hr, stringMap, drawMap, error_output);
	gl.logging(false);

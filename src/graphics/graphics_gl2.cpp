	...
	std::ostream & error_output)
{
	...
	offset_w = offset_h = 0;
	ChangeDisplay(offset_w, offset_h, resx, resy, error_output);
...
void GraphicsGL2::DrawScene(std::ostream & error_output, int offset_w, int offset_h, int w2, int h2){
	this->offset_w = offset_w;
	this->offset_h = offset_h;
	ChangeDisplay(offset_w, offset_h,   w2, h2, error_output);
	DrawScene(error_output);
}
...
void GraphicsGL2::ChangeDisplay(
	...
	std::ostream & error_output)
{
	glstate.SetViewport(sw, sh, width, height);
	...
	std::ostream & error_output)
{
	...
	for (int cubeside = 0; cubeside < cubesides; cubeside++)
	{
		...
		output.Begin(glstate, error_output, offset_w, offset_h,  w, h);

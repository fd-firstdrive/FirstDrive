...
std::set<const Bezier*> Track::GetParkingSpots() {
	return data.parkspots;
}
...
bool Track::CastRay(
	...
	int & patch_id,
	Vec3 & outtri,
	std::vector<const Bezier*> &colpatches,
	Vec3 & normal) const
{
	bool col = false;
	for (std::list <RoadStrip>::const_iterator i = data.roads.begin(); i != data.roads.end(); ++i)
	{
		...
		if (i->Collide(origin, direction, seglen, patch_id, coltri, colbez, colnorm))
		{
			if (!col || (coltri - origin).MagnitudeSquared() < (outtri - origin).MagnitudeSquared())
			{
				outtri = coltri;
				normal = colnorm;
				colpatches.push_back(colbez);
			}
			col = true;
		}
	}
    std::reverse(colpatches.begin(), colpatches.end());
	return col;
}

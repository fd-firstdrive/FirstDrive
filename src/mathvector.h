...
public:
	...
	inline bool operator== (const MathVector <T2, 3> & other) const
	{
		...
		bool eqX = (v.x == other[0]);
		bool eqY = (v.y == other[1]);
		bool eqZ = (v.z == other[2]);
	...
	inline bool operator% (const MathVector <T2, 3> & other) const	{
		...
		bool eqX = std::fabs(v.x - other[0]) < 0.1;
		bool eqY = std::fabs(v.y - other[1]) < 0.1;
		bool eqZ = std::fabs(v.z - other[2]) < 0.1;

...
AiCar * AiCarStandardFactory::Create(const CarDynamics * car, float difficulty, double MaxSpeed)
{
	return new AiCarStandard(car, difficulty, MaxSpeed);
}

AiCarStandard::AiCarStandard(const CarDynamics * new_car, float new_difficulty, double MaxSpeed) :
	AiCar(new_car, new_difficulty),
	longitude_mu(0.9),
	lateral_mu(0.9),
	last_patch(NULL),
	use_racingline(false)	//use_racingline(true)
{
	assert(car);
...
{
	if (new_value - old_value > rate_limit_pos)		  return old_value + rate_limit_pos;
	else if (new_value - old_value < -rate_limit_neg) return old_value - rate_limit_neg;
...
void AiCarStandard::SetPatch(){
	const Bezier * myPatch = GetCurrentPatch(car);
	if (myPatch != NULL){	
		if (myPatch->GetBezPatches() != NumPatches) {
			NumPatches = myPatch->GetBezPatches();
			if (NumPatches < 2) 
				idxP = 0;
			else 
				idxP = rand() % NumPatches;
		}
	}
	std::cout << "\t\t --> " << idxP << " / " << NumPatches << std::endl;
}
...
double AiCarStandard::GetPatchRadius(const Bezier & patch) {
	return GetPatchRadius(patch, 0);
}


double AiCarStandard::GetPatchRadius(const Bezier & patch, int idx){
	if (patch.GetNextPatch(idx) != NULL) {
		if (patch.GetNextPatch()->GetNextPatch() != NULL) {
...
Bezier AiCarStandard::RevisePatch(const Bezier * origpatch, bool use_racingline)
{
	...
	if (patch.GetNextPatch() == NULL) return patch;
...
bool AiCarStandard::Recover(float dt, const Bezier * /*patch*/)
{
	...
	if (car->GetVelocity().length2() < 1)	{
		...
		float dist = RayCastDistance( Vec3(0, 1, 0), max_dist);
...
float AiCarStandard::RayCastDistance(Vec3 direction, float max_length)
{
	btVector3 pos = car->GetPosition();
	btVector3 dir = car->LocalToWorld(ToBulletVector(direction)) - pos;

	CollisionContact contact;
	car->getDynamicsWorld()->castRay(pos, dir, max_length,	&car->getCollisionObject(),	contact);
...
void AiCarStandard::UpdateGasBrake()
{
	...
	if (car->GetEngine().GetRPM() < car->GetEngine().GetStallRPM())
		inputs[CarInput::START_ENGINE] = 0.6;
	else
	...
	float maxlookahead = 2*CalcBrakeDist(currentspeed, 0.0, longitude_mu);
	...
	while (dist_checked < maxlookahead)	{
		...
		if (!patch_to_check.GetNextPatch())		{
			// if there is no next patch(probably a non-closed track), brake! could be a wall
			brake_value = 0.0;
			gas_value = 0.0;
		...
		brake_dist = 1.5*CalcBrakeDist(currentspeed, speed_limit, longitude_mu);
		...
		dist_now = RayCastDistance( Vec3(0, 1, 0), 100);
		if (dist_now <= 5 ) {
			gas_value   = 0.0;
		}
		
		/*
		std::cout << "Distance to wall: [" << dist_now << "]\n";
		if (10 <= dist_now and dist_now <= 20 and std::abs(last_steer) < 0.05 ) {
			if (dist_now <= dist_prev){	//decreasing
				brake_value = std::min(0.5, brake_value + 0.00008);
				gas_value   = std::max(0.6, gas_value   - 0.00005);
			}
			else { //increasing
				brake_value = std::max(0.0, brake_value - 0.00005);
				gas_value   = std::min(1.0, gas_value   + 0.00008);
			}
		}*/
		dist_prev = dist_now;
	...
	if (car->GetSpeedMPS() < 3 and gas_value > 0.8) gas_value *= -1;

	std::cout << "gas --> \t" << gas_value << "\t" << brake_value  << " |\t" <<  " " << maxlookahead << std::endl;
...
float AiCarStandard::CalcSpeedLimit(const Bezier * patch, const Bezier * nextpatch, float friction, float extraradius=0) {
	...
	if (patch != NULL) {
		if (idxP==NumPatches and idxP != 0) idxP--;

		double adjusted_radius = GetPatchRadius(*patch, idxP);
		if (nextpatch) { 
			SetPatch();
			if (GetPatchRadius(*nextpatch) > adjusted_radius &&	GetPatchRadius(*patch, idxP) > LOOKAHEAD_MIN_RADIUS)	{
				adjusted_radius += extraradius;
			}
		}

		//float v1 = sqrt(friction * GRAVITY * adjusted_radius); //no downforce
...
float AiCarStandard::CalcBrakeDist(float current_speed, float allowed_speed, float friction)
{
	...
	float d = 2.0*(-(car->GetAerodynamicDownforceCoefficient()) * friction +
	...
	return (-log((c + v2sqr * d) * d) / (c + v1sqr*d));
}

void AiCarStandard::UpdateSteer(float dt) {
#ifdef VISUALIZE_AI_DEBUG
	steerlook.clear();
#endif
	...
	if (!curr_patch_ptr)
	{
		...
		else if (Recover( dt, curr_patch_ptr)) {
			std::cerr << "Recovering...\n";
			return;
		}
	...
	if (!curr_patch.GetNextPatch(idxP)) return;

	Bezier next_patch = RevisePatch(curr_patch.GetNextPatch(idxP), use_racingline);
	...
	while (length < lookahead)	{
		...
		length += GetPatchDirection(next_patch).Magnitude()*1.0;
	...
	angle += SteerAwayFromOthers( ); //sum in traffic avoidance bias
...
steer_value*=0.8;
	...
	inputs[CarInput::THROTTLE] = inputs[CarInput::THROTTLE] ;//- 0.1*(steer_value);
	inputs[CarInput::BRAKE]    = inputs[CarInput::BRAKE] ;//+ 0.1*(steer_value);

...
// #include "ai_car_experimental.h" USELESS
...
Ai::Ai() : empty_input(CarInput::INVALID, 0.0)
{
	...
	//AddFactory("traffic", new AiCarTrafficFactory());
}
...
void Ai::AddCar(const CarDynamics * car, float difficulty, const std::string & type, const double & MaxSpeed)
{
	assert(car);
	...
	AiCar * aicar = factory->Create(car, difficulty, MaxSpeed);

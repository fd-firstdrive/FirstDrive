...
void Game::Start(std::list <std::string> & args)
{
	...
	info_output << "Starting FirstDrive: " << VERSION << ", Revision: " << REVISION << ", O/S: " << OS_NAME << std::endl;
	...
	if (benchmode)
	{
		...
		if (!NewGame(false, false, race_laps))
		{
...
bool Game::InitCoreSubsystems()
{
	...
	{
		info_output << "The last FirstDrive startup was unsuccessful.\n";
		info_output << "Settings have been set to failsafe defaults.\n";
		info_output << "Your original FirstDrive.config file was backed up to FirstDrive.config.backup" << std::endl;
	...
	std::cout << render_ver << " is my RENDERER \n";
	...
	for (int i = 0; i < renderer_count; i++)
	{
		...
		{
			//std::cout << "  * GL3 failed, rolling back to GL2... \n";
			delete graphics;
			graphics = NULL;
			render_ver = "gl2";
			using_gl3 = false;
...
void Game::Draw(float dt) {
	...
	std::vector<SceneNode*> nodes; nodes.reserve(5);
...
// RENDERER STEP 1: DRAW FULL SCENE
	// Check if a camera is active (std case)
	if (active_camera)	{


		/*FOG TO BE PROPERLY INCLUDED
		if (true) {
	...
	/*if (gui.GetInGame() and gui.GetActivePageName() == "NoHud") {
		std::cout << "PAGE: " << gui.GetActivePageName() << " --> " << settings.GetHUD() << std::endl;
		gui.ActivatePage(settings.GetHUD(), 0.0f, error_output);
	}
	*/
	...
	graphics->DrawScene(error_output, 0,0, settings.GetResolutionX(), settings.GetResolutionY());
	PROFILER.endBlock("render draw");



// RENDERER STEP 2: DRAW REAR VIEW MIRROR
	if ( settings.GetCamera()==1 and !replay.GetPlaying() and not inGarage and gui.GetInGame() ) { 
		PROFILER.beginBlock("scenegraph");
		nodes.clear(); 		nodes.reserve(5);
		nodes.push_back(&dynamicsdraw.getNode());
		//nodes.push_back(&tire_smoke.GetNode());
		...
		for (std::vector<CarGraphics>::iterator it = car_graphics.begin(); it != car_graphics.end(); ++it){
			if (&(*it) != &car_graphics[player_car_id]) { //no need to put ourselves here :)
				graphics->AddDynamicNode(it->GetNode());
			}
		}
		...
		UpdateTrackMap(false);

		//gui.ActivatePrevPage();
		//gui.ActivatePage("NoHud", 0.0f, error_output);

		//if (active_camera) {
			CarGraphics & car_gfx = car_graphics[player_car_id];

			// Setting up RVM cam
			Camera * rvm_camera = car_gfx.GetCameras()[1];
			float fov = rvm_camera->GetFOV() > 0 ? rvm_camera->GetFOV() : settings.GetFOV();

			Vec3 reflection_location = rvm_camera->GetPosition();
			if (carcontrols_local.first)
				reflection_location = ToMathVector<float>(carcontrols_local.first->GetCenterOfMass());
			...
			camlook.Rotate(M_PI_2, 0, 0, 1);
			Quat cam_orientation = -(rvm_camera->GetOrientation() * camlook);

			Vec3 rvm = rvm_camera->GetPosition();
			rvm[0] += 0.4;
			rvm[1] -= 0.4;
			rvm[2] += 1.2;
			graphics->SetupScene(
				fov, 
				settings.GetViewDistance(),
				rvm, //active_camera->GetPosition(),
				cam_orientation,
				reflection_location,
				error_output
			);
			
		PROFILER.beginBlock("render draw rvm");
		int rvmX = 320;
		int rvmY = 180;

			//graphics->DrawScene(error_output, 0,0, settings.GetResolutionX(), settings.GetResolutionY());
			graphics->DrawScene(error_output, settings.GetResolutionX()-rvmX-100, settings.GetResolutionY()-rvmY-100, rvmX, rvmY);
			//graphics->DrawScene(error_output, 0, 0, rvmX, rvmY);
			UpdateTrackMap();
			//gui.ActivatePrevPage();
		PROFILER.endBlock("render draw rvm");
...
void Game::UpdateTimer()
{
	...
	for (int i = 0; i != car_dynamics.size(); ++i)
	{
		...
		if (track.GetSectors() > 0)
		{
			...
			nextsector = rand() % track.GetSectors();
			for (int p = 0; p < 4; ++p) {
...
void Game::UpdateTrackMap(bool isActive)
{
	std::list <std::pair<Vec3, bool> > carpositions;
	for (int i = 0; i != car_dynamics.size(); ++i)
	{
	...
	trackmap.Update(isActive, carpositions);
}
...
void Game::ProcessCarInputs(const size_t carid)
{
	...
	else if (carcontrols_local.first == &car)
	{
		...
		// It allows to activate the AI on the player car with F11 button.
		...
		if (buttonPressed != eventsystem.GetKeyState(SDLK_F11).just_down)
		{
	...
	std::string currentInfraction = "";
	bool parked = false;
	// Car rule manager START
	if (carid == player_car_id) {
		currentInfraction = controlInfractions(car);

		if (not ruleNotSpokenTTS and currentInfraction != "" and not isExamMode) {
			std::thread t_speak(speak, currentInfraction); t_speak.detach();
			ruleNotSpokenTTS = true;
		}

		else if (settings.GetTrack() == "ex1_Park" and not ruleNotSpokenTTS) {
			newRule = "Park right now. Get behind the second light of a car. Then, once you see it on the mirror, start going backwards and steer right as you keep going.";
			ruleSet = true;
			goParking = true;
			ruleNotSpokenTTS = true;
			std::thread t_speak(speak, newRule); t_speak.detach();
		}
		else if (settings.GetTrack() != "ex1_Park") getNextRule(car);

		parked = checkParking(car);
	} 
	// Car rule manager END



	// Record car state.
	if (replay.GetRecording())	replay.RecordFrame(carid, carinputs, car, currentInfraction);

	if (carid == player_car_id and settings.GetHUD() != "NoHud")
		UpdateHUD(carid, carinputs, currentInfraction);


	if (goParking and parked and car.GetCurrentGear() == 0 /*and car->GetSpeedMPS() < 0.2*/) {
		if (scoreDrive>=0) 	{std::thread t_speak(speak, "Good! You have passed the test! Congratulations."); t_speak.detach();}
		else {std::thread t_speak(speak, "You have finished and failed the test..."); t_speak.detach();}
		
		LoadGarage();
	};
	//std::cout << "Current Score: " << scoreDrive << std::endl;
}

bool Game::checkParking(CarDynamics & car) {
	const Bezier * myPatch[4];
	myPatch[0] = car.GetWheelContact(FRONT_LEFT).GetPatch(); myPatch[1] = car.GetWheelContact(FRONT_RIGHT).GetPatch();
	myPatch[2] = car.GetWheelContact(REAR_LEFT).GetPatch();	 myPatch[3] = car.GetWheelContact(REAR_RIGHT).GetPatch();
	if (myPatch[0]==myPatch[1] and myPatch[1]==myPatch[2] and myPatch[2]==myPatch[3] and car.GetSpeedMPS() < 0.2 ) {
		if (parkspots.find(myPatch[0]) != parkspots.end() /*and carinputs[CarInput::HANDBRAKE]*/){
			return true;
		}
	}
	//std::cout << parkspots.size() <<std::endl;
	return false;
}


std::string Game::controlInfractions(CarDynamics & car) {
	std::string infraction = "";
	//INFRACTION CONTROL
	/////////////////////////////////////////////////////////////////////////
	std::vector<std::string> infractions(0);
	if (settings.GetTrack() == "ex3_Highway" or settings.GetTrack() == "weekend") {
		if (car.GetSpeedMPS()*3.6 >= 120 and insideInfraction) { //bolzano
			infraction = "SEVERE: Exceeding 120 km/h!";
			scoreDrive -= 30;
			insideInfraction = false;
		}
		else if (car.GetSpeedMPS()*3.6 >= 90 and !insideInfraction) { 
			infraction = "Exceeding 90 km/h!";
			scoreDrive -= 10;
			insideInfraction = true;
		}
		else if (car.GetSpeedMPS()*3.6 < 90) {
			insideInfraction = false;
		}
	}
	else {
		if (car.GetSpeedMPS()*3.6 >= 80 and insideInfraction) { //bolzano
			infraction = "SEVERE: Exceeding 80 km/h!";
			scoreDrive -= 30;
			insideInfraction = false;
		}
		else if (car.GetSpeedMPS()*3.6 >= 60 and !insideInfraction) {
			infraction = "Exceeding 60 km/h!";
			scoreDrive -= 10;
			insideInfraction = true;
		}
		else if (car.GetSpeedMPS()*3.6 < 60) {
			insideInfraction = false;
		}
	...
	if (setSignalMode=="STOP" and car.GetSpeedMPS() <= 0.05) {
		signalIsFollowed = true;
		//infraction= "GOOD! YOU HAVE STOPPED!\n";
	}
	else if (setSignalMode=="CEDA" and car.GetSpeedMPS() <= 2.78) {
		signalIsFollowed = true;
		//infraction= "GOOD! YOU HAVE GIVEN WAY!\n";
	}


	if (myPatch[0]!=NULL and myPatch[0]==myPatch[1] and myPatch[2]==myPatch[3] and myPatch[0]!=myPatch[2]) {
		if (myPatch[0] != NULL or myPatch[0]->GetSignal() != "") {
			setSignalMode = myPatch[0]->GetSignal();
			std::cout << "Next signal:  " << setSignalMode << std::endl;
			signalIsFollowed = false;
		}
		else if (myPatch[2] != NULL){
			if (myPatch[2]->GetSignal() == "STOP" and setSignalMode == "STOP"){
				if (!signalIsFollowed) {
					infraction = "STOP Failed";   //infractions = "SEVERE: You skipped that STOP";
					scoreDrive -= 10;
				}
				else infraction = "STOP Successful";   //std::cout << "GOOD!\n";
			}
			else if (myPatch[2]->GetSignal() == "CEDA" and setSignalMode == "CEDA" ){
				if (!signalIsFollowed) {
					infraction = "GIVEWAY Failed";   //infractions = "SEVERE: You skipped that STOP";
					scoreDrive -= 10;
				}
				else infraction = "GIVEWAY Successful";   //std::cout << "GOOD!\n";
			}
			setSignalMode = "";
		}
	}
	/*bool indicatorsSet = 1;//carinputs[CarInput::INDICATOR_LEFT] or carinputs[CarInput::INDICATOR_RIGHT];
	if (myPatch[0]!=NULL and myPatch[0]!=myPatch[1] and myPatch[2]!=myPatch[3] and myPatch[0]==myPatch[2] and myPatch[1]==myPatch[3]) { //change way
		Vec3 dir0 = ((myPatch[0]->GetPoint(3,0) + myPatch[0]->GetPoint(3,3)) - (myPatch[0]->GetPoint(0,0) + myPatch[0]->GetPoint(0,3))) * 0.25;
		Vec3 dir1 = ((myPatch[1]->GetPoint(3,0) + myPatch[1]->GetPoint(3,3)) - (myPatch[1]->GetPoint(0,0) + myPatch[1]->GetPoint(0,3))) * 0.25;
		if (dir0 % dir1 and !indicatorsSet) {
			infraction = "Wrong way! No indicators set";
			scoreDrive -= 10;
		}
		else if (!indicatorsSet) infraction = "Not using indicators!";
	}*/

	if (myPatch[0]!=NULL and myPatch[2]!=NULL and myPatch[0]==myPatch[1] and myPatch[2]==myPatch[3] and myPatch[0]!=myPatch[2] and myPatch[1]!=myPatch[3] and ruleSet and myPath >= 0) { //change way
		ruleSet = false;
		if (myPath < myPatch[2]->GetAllPatches().size() and myPatch[2]->GetAllPatches()[myPath] != myPatch[0]) {
			infraction = "Wrong direction...";
			scoreDrive -= 50;
		}
		//else infraction = "Good job!";
	}
	return infraction;
}
...
void Game::speak(std::string sentence){
	int i=0;
	//festival_say_text(sentence.c_str());
	
	std::string say = "echo \"" + sentence + "\" | festival --tts";
	//system( say.c_str() ); // If festival somehow doesn't compile...
}	

void Game::getNextRule(CarDynamics & car) {
	const Bezier * myPatch[4];
	myPatch[0] = car.GetWheelContact(FRONT_LEFT).GetPatch(); myPatch[1] = car.GetWheelContact(FRONT_RIGHT).GetPatch();
	myPatch[2] = car.GetWheelContact(REAR_LEFT).GetPatch();	 myPatch[3] = car.GetWheelContact(REAR_RIGHT).GetPatch();
	...
	/*if (myPatch[0]!=NULL) {
		std::cout << "My patch : " << myPatch[0]->idx << "\t | Num of patches = " << (*myPatch[0]).GetBezPatches() << std::endl;
		for (int i = 0; i< (*myPatch[0]).GetBezPatches() ; ++i) {
			std::cout << "\t Next["<<i<<"] = " << myPatch[0]->GetAllPatches()[i]->idx << std::endl;
		}
	}
	else std::cout << "SUCCESSORS: ---" << std::endl;*/

	if (myPatch[0]==NULL) return;

	if (!ruleSet and ruleCounter < 10 and myPatch[0] == myPatch[1] and myPatch[0]->GetAllPatches().size() > 1 ) {
		int SIZE = myPatch[0]->GetAllPatches().size();
		std::string choice; 
		myPath = -1;
		if (SIZE == 2){
			myPath = rand() % 2;
			if (myPath == 0) choice = "Turn left. If not possible, go straight.";
			else /*if (myPath == 1)*/     choice = "Go straight. If not possible, turn right.";
			
		} 
		else /* (if SIZE==3) */{
			if (myPath == 0) choice = "Turn left.";
			else if (myPath == 1)     choice = "Keep going straight.";
			else/* if(myPath == 2) */ choice = "Turn right.";
		}

		newRule = "On this corner, " + choice;
		setGoRight = (myPath != -1) and (SIZE-1 == myPath);
		ruleSet = (ruleCounter%2 == 0);
		++ruleCounter;
		ruleNotSpokenTTS = true;
	}
	else if (!ruleSet and ruleCounter >= 20) {
		
		newRule = "Park whenever possible.";
		ruleSet = true;
		goParking = true;
		ruleNotSpokenTTS = true;
	}
	...
	if (ruleSet and ruleNotSpokenTTS) {
		//speak és el punter a la funció que crida a l'API de Festival.
		//newRule és un char* que conté el text que es parlarà.
		std::cout << "\t I SPOKE 0...\n";
		ruleNotSpokenTTS = false;
		std::thread t_speak(speak, newRule); t_speak.detach();
	}
}
...
void Game::UpdateHUD(const size_t carid, const std::vector<float> & carinputs, std::string & infraction)
{
	...
	std::ostringstream absstr, tcsstr, listr, ristr;
	absstr << (car.GetABSActive() ? 1.0 : 0.3);
	tcsstr << (car.GetTCSActive() ? 1.0 : 0.3);

	//listr << (car.GetFuelAmount() ? 0.3 : 1.0);
	//ristr << ((car.GetNosAmount() && carinputs[CarInput::NOS]) ? 1.0 : 0.3);

	//Can't have both of them at the same time, unless noted!
	if (carinputs[CarInput::INDICATOR_LEFT])  {
		isLeftIndicatorOn  = !isLeftIndicatorOn; //toggle
		if (isRightIndicatorOn) isRightIndicatorOn = false;
	}
	else if (carinputs[CarInput::INDICATOR_RIGHT]) {
		isRightIndicatorOn = !isRightIndicatorOn; //toggle
		if (isLeftIndicatorOn) isLeftIndicatorOn = false;
	}

	emergencyIsOn = carinputs[CarInput::EMERGENCY];

	//Check if wheel is centered
	if (abs(carinputs[CarInput::STEER_RIGHT] - carinputs[CarInput::STEER_LEFT]) > 10) {
		isLeftIndicatorOn  = false;
		isRightIndicatorOn = false;
	}

	if (not emergencyIsOn){
		listr << (isLeftIndicatorOn  && (time_ind < 75) ? 1.0 : 0.3);
		ristr << (isRightIndicatorOn && (time_ind < 75) ? 1.0 : 0.3);
	}
	else {
		listr << (emergencyIsOn && (time_ind < 75) ? 1.0 : 0.3);
		ristr << (emergencyIsOn && (time_ind < 75) ? 1.0 : 0.3);
	}

	++time_ind;
	if (time_ind >= 150) time_ind = 0;
	...
	signal_score(std::to_string(scoreDrive));//	signal_score(scorestr.str());
	signal_message(msgstr.str()); signal_message(infraction);
	...
	signal_lefti(listr.str());
	signal_righti(ristr.str());
}
...
bool Game::NewGame(bool playreplay, bool addopponents, int num_laps)
{
	...
	{
		if (!LoadCar(car_info[i], track.GetStart(i).first, track.GetStart(i).second, MaxSpeed, sound.Enabled()))
			return false;
	}

	// Load timer.
	float pretime = (num_laps > 0) ? 1.0f : 0.0f;
	...
	// Record a replay.
	if (/*settings.GetRecordReplay() and*/ !playreplay)
	{
	...
	// Set up GUI.
	gui.SetInGame(true);
	gui.ActivatePage("Hud", 0.0f, error_output);
	...
	const bool sound_enabled)
{
	...
	if (!isai)
		...
		ai.AddCar(&car, info.ailevel, info.driver, MaxSpeed);
...
bool Game::LoadTrack(const std::string & trackname)
{
		...
		graphics->GetShadows()))
	{
		...
		parkspots = track.GetParkingSpots();
		std::cout << "Park spots: " << parkspots.size() << std::endl;
	}
	bool success = true;
	int count = 0;
...
void Game::SetGarageCar()
{
	...
	double ZERO = 0.0;
	if (LoadCar(car_info[car_edit_id], car_pos, car_rot, ZERO, false))
	{
...
void Game::LeaveGame()
{
	...
	} else info_output << "I wasn't recording...\n";
...
void Game::InitSignalMap(std::map<std::string, Signal1<const std::string &>*> & signalmap)
{
	...
	signalmap["car.indicator_left"]  = &signal_lefti;
	signalmap["car.indicator_right"] = &signal_righti;
}

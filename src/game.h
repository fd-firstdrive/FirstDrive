...
private:
	...
	//NEW
	std::string controlInfractions(CarDynamics & car);
	std::set<const Bezier*> parkspots;
	bool checkParking(CarDynamics & car);
	void getNextRule(CarDynamics & car);
	static void speak(std::string sentence);
	...
	void UpdateHUD(const size_t carid, const std::vector<float> & carinputs, std::string & infraction);
	...
	void UpdateTrackMap();
	void UpdateTrackMap(bool isActive);
	...
	Signal1<const std::string &> signal_lefti;
	Signal1<const std::string &> signal_righti;
	...
	bool isLeftIndicatorOn, isRightIndicatorOn, emergencyIsOn;
	...
	// EXTRA ADDITIONS FIRSTDRIVE

	std::string setSignalMode;
	bool signalIsFollowed;
	...
	std::string newRule;
	int ruleCounter; //Total rules found
	int passCounter; //Total rules passed
	int scoreDrive; //Total score in the end
	int myPath; //Total score in the end

	bool ruleEvaluated; 
	bool rulePass; //Cannot be 1 if ruleEvaluated==0 !!!
	bool ruleNotSpokenTTS;
	bool isExamMode;
	bool insideInfraction;
	float oldSpeed;
};

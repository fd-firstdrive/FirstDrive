...
bool Track::Loader::BeginLoad()
{
	...
	if (!LoadStartPositions(info)) return false;
	std::cout << "Starts loaded. \n";

	if (!LoadLapSections(info))  return false;
	std::cout << "Lap sections, stops, giveways and park spots loaded. \n";

	if (!BeginObjectLoad())  return false;
	std::cout << "Obj loaded. \n";
...
std::pair<bool, bool> Track::Loader::ContinueOld()
{
	...
	//std::cout << object.texture << std::endl;
...
void Track::Loader::findSuccessors() {
	std::cout << "SUCCESSORS: \n";
	std::vector<std::pair<RoadPatch*, RoadPatch*> > ExtremesRoads(0);
	
	// Obtain extremes from all RoadStrips
	int j = 0;
	for (std::list<RoadStrip>::iterator i = data.roads.begin(); i != data.roads.end(); ++i) { 
		RoadStrip myRoad = (*i);
		
		RoadPatch* ini = &myRoad.GetPatches()[0];
		RoadPatch* end = &myRoad.GetPatches()[myRoad.GetPatches().size() - 1];
		std::pair<RoadPatch*, RoadPatch*> myPair(ini, end);
		ExtremesRoads.push_back( myPair );
		
		std::cout << "\troad [" << j << "] of size = " << myRoad.GetPatches().size() << "\n";
		++j;
	}
	j = 0;
	for (std::list<RoadStrip>::iterator it1 = data.roads.begin(); it1 != data.roads.end(); ++it1, ++j) { 
		RoadPatch* connector = ExtremesRoads[j].second;
		int k = 0;
		for (std::list<RoadStrip>::iterator it2 = data.roads.begin(); it2 != data.roads.end(); ++it2, ++k) { 
			RoadPatch* connectee = ExtremesRoads[k].first;
			
			if (j != k) {
				std::cout << "\n FIN:\n";
				//std::cout << '\n';
				for (int i = 0; i < 4; ++i) {
					std::cout << i << " --> " << connector->GetPatch().GetPoint(0, i) << std::endl;
				} std::cout << " INI:\n";
				for (int i = 0; i < 4; ++i) {
					std::cout << i << " --> " << connectee->GetPatch().GetPoint(3, i) << std::endl;
				}
				std::cout << "\n----------------------\n";
			}

			if  (j != k 
			  and (connector->GetPatch().GetPoint(0, 0) % connectee->GetPatch().GetPoint(3, 0) )
			  and (connector->GetPatch().GetPoint(0, 1) % connectee->GetPatch().GetPoint(3, 1) )
			  and (connector->GetPatch().GetPoint(0, 2) % connectee->GetPatch().GetPoint(3, 2) )
			  and (connector->GetPatch().GetPoint(0, 3) % connectee->GetPatch().GetPoint(3, 3) )){

				std::cout << "\t FOUND 1 Fin -> Ini [" << j << "," << k << "]"<< std::endl;
				std::cout << "\t                    [" <<  connector->GetPatch().GetId() << "," <<  connectee->GetPatch().GetId() << "]"<< std::endl;
				it1->addSuccessor( &*connectee );
				//it2->addSuccessor( &*connector );
			}
			else if  (j != k 
			  and (connector->GetPatch().GetPoint(0, 1) % connectee->GetPatch().GetPoint(3, 1) )
			  and (connector->GetPatch().GetPoint(0, 2) % connectee->GetPatch().GetPoint(3, 2) )
			  and (connector->GetPatch().GetPoint(0, 3) % connectee->GetPatch().GetPoint(3, 3) )){
				...
				std::cout << "\t FOUND 2 Fin -> Ini [" << j << "," << k << "]"<< std::endl;
				std::cout << "\t                    [" <<  connector->GetPatch().GetId() << "," <<  connectee->GetPatch().GetId() << "]"<< std::endl;
				it1->addSuccessor( &*connectee );
				//it2->addSuccessor( &*connector );
			}
...
bool Track::Loader::LoadLapSections(const PTree & info)
{
		...
		for (int l = 0; l < lapmarkers; l++) {
			...
			for (std::list<RoadStrip>::iterator i = data.roads.begin(); i != data.roads.end(); ++i, ++curroad) {
				if (curroad == roadid) {
					...
					i->GetPatches()[patchid].GetPatch().SetSignalToStop();
					break;
				}
			}
		}
	...
	lapmarkers = 0;
	if (info.get("number of stops", lapmarkers)) {
		std::cout << "Reading stops... " << lapmarkers << "\n";
		for (int l = 0; l < lapmarkers; l++) {
			std::vector<float> lapraw(3);
			std::ostringstream lapname;
			lapname << "stop " << l;
			...
			for (std::list<RoadStrip>::iterator i = data.roads.begin(); i != data.roads.end(); ++i, ++curroad) {
				if (curroad == roadid) {
					...
					data.stop.push_back(&i->GetPatches()[patchid].GetPatch());
	...
	lapmarkers = 0;
	if (info.get("number of giveways", lapmarkers)) {
		for (int l = 0; l < lapmarkers; l++) {
			std::vector<float> lapraw(3);
			std::ostringstream lapname;
			lapname << "giveway " << l;
			...
			for (std::list<RoadStrip>::iterator i = data.roads.begin(); i != data.roads.end(); ++i, ++curroad) {
				if (curroad == roadid) {
					int num_patches = i->GetPatches().size();
					assert(patchid < num_patches);
					data.ceda.push_back(&i->GetPatches()[patchid].GetPatch());
					i->GetPatches()[patchid].GetPatch().SetSignalToCeda();
					break;
				}
			}
		}
	...
	lapmarkers = 0;
	if (info.get("number of parking spots", lapmarkers)) {
		std::cout << "\t\tPARKS: " << lapmarkers << "/" << data.roads.size() << std::endl; 
		for (int l = 0; l < lapmarkers; l++) {
			std::vector<float> lapraw(3);
			std::ostringstream lapname;
			lapname << "park " << l;
			...
			for (std::list<RoadStrip>::iterator i = data.roads.begin(); i != data.roads.end(); ++i, ++curroad) {
				if (curroad == l /*roadid*/) {
					int num_patches = i->GetPatches().size();
					std::cout << "\t\t   Adding park = " << l << " " << patchid << " " << roadid << std::endl; 
					assert(patchid < num_patches);

					data.park.push_back(&i->GetPatches()[patchid].GetPatch()); //maybe redundant, see Set
					data.parkspots.insert(&i->GetPatches()[patchid].GetPatch()); // SET!
					//parking.insert(  &i->GetPatches()[patchid].GetPatch()  );
	...
	Bezier* curr_patch = NULL;
	if (start_patch->next_patch.size()> 0)
		curr_patch = start_patch->next_patch[0];
	...
	while ( curr_patch && curr_patch != start_patch)
	{
		...
		curr_patch = curr_patch->next_patch[0];
		...
		if (start_patch->next_patch.size()> 0){
			...
			while (curr_patch != NULL) 
				curr_patch = start_patch->next_patch[1];
			if (curr_patch == NULL) 
				{ info_output << "ERROR!\n"; exit(0);}
		}*/
		...
		Bezier* curr_patch = NULL;
		if (start_patch->next_patch.size()> 0){
			for (int i = 0; i < start_patch->next_patch.size() and curr_patch != NULL; ++i) {
				curr_patch = start_patch->next_patch[i];
			} 
		}

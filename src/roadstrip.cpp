...
void RoadStrip::addSuccessor(RoadPatch* succ) {
	if ((successors.size() == 0) /*or (successors.size() > 0 and successors[successors.size()-1] != succ)*/) {
		std::cout << "adding...\n";
		successors.push_back(succ);

		//if (successors[successors.size()-1] != succ)std::cout << "\tto last!\n";

		patches[patches.size()-1].GetPatch().Attach( succ->GetPatch() ) ;

	}
	else std::cout << "duplicate road WON'T be added!\n";
}
	...
	Vec3 & normal) const
{
	if (patch_id >= 0 && patch_id < (int)patches.size())
	{
		...
		if (patches[patch_id].Collide(origin, direction, seglen, coltri, colnorm))
		{
			outtri = coltri;
			normal = colnorm;
			//colpatch.push_back(patches[patch_id].GetPatch());
			return true;
		}
	}

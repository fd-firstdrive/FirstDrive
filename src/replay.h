...
public:
	...
	void RecordFrame(unsigned carid, const std::vector <float> & inputs, CarDynamics & car, std::string infraction);
...
private:
	...
	public:
		...
		StateFrame(unsigned newframe, std::string newinfr);
		...
		const std::string & GetInfraction() const {
			return infraction;
		}
	...
	struct CarState
	{
		...
		void RecordFrame(const std::vector<float> & inputs, CarDynamics & car, std::string infraction);

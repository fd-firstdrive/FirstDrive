...
Bezier::Bezier() : signal("")
{
	next_patch = std::vector<Bezier *> (0);
	//next_patch = NULL;
	turn = 0;
...
void Bezier::Attach(Bezier & other, bool reverse)
{
	...
	next_patch.push_back( &other);
	std::cout << "SIZE CACHITO: " << next_patch.size() << std::endl;
	...
	if (other.next_patch.size() == 0 || reverse) other.dist_from_start = dist_from_start + d1.Magnitude();

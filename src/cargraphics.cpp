	...
	std::ostream & error_output)
{
	...
	if (cfg_wheel.get("brake", cfg_brake, error_output) && cfg_brake->get("texture", texname)) {
		float radius;
	...
	std::ostream & error_output)
{
	...
	// load brake/reverse/indicator light point light sources (optional)
	int i = 0;
	std::string istr = "0";
	...
	while (cfg.get("light-brake-"+istr, cfg_light))
	{
		...
		{
			error_output << "Failed to load brake lights." << std::endl;
			return false;
		}
	...
	while (cfg.get("light-reverse-"+istr, cfg_light)) {
		if (!LoadLight(*cfg_light, content, error_output)) {
			error_output << "Failed to load reverse lights." << std::endl;
			return false;
		}
	...
	while (cfg.get("light-indicator-"+istr, cfg_light))
	{
		std::cout << "\t-->LIGHT " << i << std::endl;
		if (!LoadLight(*cfg_light, content, error_output))
		{
			error_output << "Failed to load indicator lights." << std::endl;
			return false;
		}
	...
	if (cfg.get("light-reverse", cfg_light))
	{
		...
		{
			error_output << "Failed to load reverse ights." << std::endl;
			return false;
		}
	}
	if (cfg.get("light-indicator", cfg_light))
	{
		SceneNode & bodynoderef = topnode.GetNode(bodynode);
		if (!loadDrawable(*cfg_light, bodynoderef, 0, &indicatorlights))
		{
			error_output << "Failed to load indicator lights." << std::endl;
			return false;
		}
...
void CarGraphics::Update(const std::vector<float> & inputs)
{
	...
	left_indicator  = inputs[CarInput::INDICATOR_LEFT];
	right_indicator = inputs[CarInput::INDICATOR_RIGHT];
	/*if (inputs[CarInput::EMERGENCY]) {
		emergency_light = !emergency_light;
		std::cout << "x" << std::endl;
	}*/ emergency_light = inputs[CarInput::EMERGENCY];

	car_light       = inputs[CarInput::LIGHTS];
...
void CarGraphics::Update(const CarDynamics & dynamics)
{
	...
	if (indicatorlights.valid()) {
		Drawable & draw = bodynoderef.GetDrawList().lights_emissive.get(indicatorlights);
		bool isTurned = emergency_light && (time_ind < 75);
		//std::cout << "Number of lights: " << bodynoderef.GetDrawList().lights_emissive.get(indicatorlights) << std::endl;
		draw.SetDrawEnable(isTurned);
	}
	
	++time_ind;
	if (time_ind >= 150) time_ind = 0;
	//if (time_ind % 100 == 0) std::cout << "TIME -> " << time_ind << std::endl;

...
void Replay::RecordFrame(unsigned carid, const std::vector <float> & inputs, CarDynamics & car, std::string infraction)
{
	assert(carid < carstate.size());
	assert(unsigned(version_info.inputs_supported)== CarInput::INVALID);
	...
	if (GetRecording())
	{
		...
		carstate[carid].RecordFrame(inputs, car, infraction);
	}
}

void Replay::CarState::RecordFrame(const std::vector <float> & inputs, CarDynamics & car, std::string infraction)
{
	...
	if (frame % 30 == 0)
	{
		...
		stateframes.push_back(StateFrame(frame, infraction));
...
void Replay::CarState::ProcessPlayStateFrame(const StateFrame & frame, CarDynamics & car)
{
	...
	std::cout << frame.GetInfraction() << std::endl;
...
Replay::StateFrame::StateFrame(unsigned newframe, std::string newinfr) :
	frame(newframe),
	infraction(newinfr)
{
	// ctor

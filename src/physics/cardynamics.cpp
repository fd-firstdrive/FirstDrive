...
btScalar CarDynamics::GetSpeedMPS() const
{
	...
	btScalar wheel0 = wheel[0].GetRadius() * wheel[0].GetAngularVelocity();
	wheel0 += wheel[1].GetRadius() * wheel[1].GetAngularVelocity();
	wheel0 += wheel[2].GetRadius() * wheel[2].GetAngularVelocity();
	wheel0 += wheel[3].GetRadius() * wheel[3].GetAngularVelocity();
...
void CarDynamics::UpdateWheelContacts()
{
	...
	for (int i = 0; i < WHEEL_POSITION_SIZE; ++i)
	{
		...
		{
			// wheel separated
			std::vector<Bezier *> patches(0);
			wheel_contact[i] = CollisionContact(raystart, raydir, raylen, -1, 0, TrackSurface::None(), 0);
			//LEGACY wheel_contact[i] = CollisionContact(raystart, raydir, raylen, -1, 0, TrackSurface::None(), 0);
		}
		else
		{
...
///return actual gear
int CarDynamics::GetCurrentGear() const {
	return transmission.GetGear();
}

/*
void CarDynamics::Reverse()   const { ShiftGear(-1); }
void CarDynamics::Unreverse() const { ShiftGear(+1); }
*/
...
int CarDynamics::NextGear() const
{
	...
	if (shifted && clutch.GetPosition() == 1.0)
	{
		...
		if (driveshaft_rpm > (engine.GetRedline()*0.625) && gear > 0)
		{
			return gear + 1;
		}
	...
	//Reverse management REVIEW FOR AI
	/*
	btVector3 direction = btVector3(0, 1, 0);
	btVector3 pos = GetPosition();
	btVector3 dir = LocalToWorld(ToBulletVector(Vec3(0, 1, 0))) - pos;
	CollisionContact contact;
	//getDynamicsWorld()->castRay(pos, dir, direction, getCollisionObject(), contact);
	float depth = contact.GetDepth();
	float dist = fmin(3, depth);
	if (dist < 2.97f) return -1;
	*/
...
btScalar CarDynamics::DownshiftRPM(int gear) const
{
	...
	if (gear > 1)
	{
		...
		shift_down_point = 0.7 * peak_engine_speed / lower_gear_ratio * current_gear_ratio * 0.5;
	}
...
btScalar CarDynamics::CalculateMaxSpeed() const
{
	...
	/* // MAX SPEED FROM EVERY WHEEL
	btScalar d_s1 = wheel[FRONT_LEFT].GetRadius() / ratio;
	btScalar d_s2 = wheel[FRONT_RIGHT].GetRadius() / ratio;
	btScalar d_s3 = wheel[REAR_LEFT].GetRadius() / ratio;
	btScalar d_s4 = wheel[REAR_RIGHT].GetRadius() / ratio;

	drive_speed = drive_speed * d_s1 * d_s2 * d_s3 * d_s4;*/

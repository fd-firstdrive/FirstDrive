	...
	CollisionContact & contact) const
{
	...
	if (ray.hasHit())
	{
		...
		if (track)
		{
			...
			//if (track->CastRayOld(org, dir, length, patch_id, colpoint, b, colnormal)) //SINGLETON VERSION

			// Ray hits track? Good, add it into list of patches found by ray
			if (track->CastRay(org, dir, length, patch_id, colpoint, bz, colnormal))
			{
		...
		contact = CollisionContact(p, n, d, patch_id, bz, s, c);
		return true;
	}

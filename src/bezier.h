...
public:
	...
	bool operator==(const Bezier & other) {
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
				if (points[i][j] != other.GetPoint(i,j)) return false;
			}
		}
		return true;
	}
	...
	void ResetNextPatch() {
		if (next_patch.size() > 0) next_patch[0] = NULL;
		else next_patch = std::vector<Bezier *> (0);
	}
	...
	void Attach(std::vector<Bezier> & other) {
		for (auto o: other){
			Attach(o, false);
		}
	}
	...
	Bezier* GetNextPatch(int i) const
	{
		assert(-1 < i );
		if (i < int( next_patch.size()) and 0 < int( next_patch.size())) {	
			i = next_patch.size()-1;
			return next_patch[i];
		}
		return NULL;
	...
	int GetBezPatches() const {
		return next_patch.size();
	}

	std::vector<Bezier *> GetAllPatches() const {
		return next_patch;
	}
	...
	void SetSignal(std::string sig){
		signal = sig;
	}

	void SetSignalToStop(){
		signal = "STOP";
	}
	void SetSignalToCeda(){
		signal = "CEDA";
	}
...
private:
	...
	std::vector<Bezier *> next_patch;
	float track_radius;
	
	short turn; //-1 - this is a left turn, +1 - a right turn, 0 - straight
	std::string signal; //-1 - ceda, +1 - stop, 0 - no signal
